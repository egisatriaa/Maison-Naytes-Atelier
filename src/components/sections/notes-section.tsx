...
export function NotesSection() {
    ...
    const handleExplain = async (
        ...
        try {
            ...
            setDescription(
                'A refined note that completes this exquisite composition.',
            );
        } finally {
            setExplaining(null);
        }
            ...
            <div className="max-w-[1200px] mx-auto px-8">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
                    <div>
                        ...
                        <h3 className="text-4xl md:text-5xl font-headline">
                            Fragrance Notes
                        </h3>
                    </div>
                        ...
                        <div className="space-y-4">
                            {activeVariant.notes.top.map((note) => (
                                <NoteItem
                                    key={note}
                                    note={note}
                                    onExplain={() => handleExplain(note, 'Top')}
                                    isExplaining={explaining === note}
                                />
                            ))}
                        </div>
                    </div>
...
function NoteItem({
    ...
    return (
        <SpotlightCard
            className="bg-muted/30 hover:bg-muted/50 transition-colors cursor-pointer group border border-white/5"
            onClick={onExplain}
            spotlightColor="rgba(209, 170, 91, 0.6)"
        >
